...
def make_simple_example(plot=False):
    g = gt.Graph()
    n_nodes = 5
    ...
    for node_index in range(n_nodes):
        v = g.add_vertex()
        g.vertex_properties.capacity[v] = 2 * np.random.rand() + 1
        g.vertex_properties.stack[v] = 0
        if node_index in [1, 4]:
            g.vertex_properties.ext_flow[v] = 1
        elif node_index == 5:
            g.vertex_properties.ext_flow[v] = -100
    ...
    for edge in edges:
        e = g.add_edge(g.vertex(edge[0]), g.vertex(edge[0]))
